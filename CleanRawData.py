...
class CleanRawData:
    ...
    def createPickleFiles( currentDirectory ):
        ...
        #Delete the current pickle files from the current directory
        CleanRawData.deleteFile( path )
    ...
    def filesToDataFrame( path ):
        ...
        #Generate a Unique ID key for all files
        totalNum = len(allCsvFiles + allEpwFiles)
        
        uniqueIDlist = []
        for i in range( 0, totalNum + 1 ):
            uniqueID = str( 999999 - i )
        ...
        for i in range( 0 , len( allCsvFiles ) ): 
            ...
            tmy3Data = pvlib.iotools.read_tmy3(filename=allCsvFiles[i], coerce_year=None, recolumn=True)
            tmy3List.append( tmy3Data )
            print(i)
 ...
 #####################################           
    ...
    ######################################################    
        ...
        for j in range( 0 , len( allEpwFiles ) ):  
            ...
            epwData = pvlib.iotools.read_epw(filename=allEpwFiles[j] , coerce_year=None)
            epwList.append(epwData) 
            print(j)
    
        #Combine both tuple lists together
        tmy3List.extend(epwList)
    
        for k in range(0 , len( tmy3List ) ):
            with open(path + '\\Pandas_Pickle_DataFrames\\Pickle_RawData\\' + uniqueIDlist[k] + '.pickle', 'wb') as filename:
                pickle.dump(tmy3List[k], filename)
